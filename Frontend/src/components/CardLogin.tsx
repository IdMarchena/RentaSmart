...
export const CardLogin = ({ onClose }: CardLoginProps) => {
    ...
    const { executeOnSuccess, closeModal,isOpenRegister,openRegisterModal } = useAuthModal();
    ...
    return (
        <>
            <div className="relative w-[350px] h-[600px] flex flex-col justify-center items-center rounded-[20px] p-6 shadow-[0px_10px_10px_rgba(0,0,0,0.2)] bg-[#FEFCEC] overflow-hidden">
                ...
                <form onSubmit={handlelogin} className="w-full h-auto flex flex-col items-center justify-center gap-5 z-10">
                    ...
                    <div className="flex flex-row items-center justify-center gap-2 z-10">
                        ...
                        <p className="text-[11px] text-[#EB8369] font-bold cursor-pointer" onClick={() => openRegisterModal()}>Crear</p>
                    </div>
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className="w-[60%] h-[40px] rounded-[10px] bg-[#EB8369] shadow-[0px_5px_10px_rgba(0,0,0,0.2)] cursor-pointer hover:bg-[#d67359] transition-colors duration-200">
                        <span className="font-semibold text-sm text-white">{loading ? 'Cargando...' : 'Iniciar Sesión'}</span>
                    </button>
                    {error && (
            ...
            {isOpenRegister && (
                <div className="fixed inset-0 z-50 flex items-center justify-center">
                    ...
                    <div className="relative z-10">
                        <CardRegister onClose={closeModal} />
                    </div>
                </div>
            )}
        </>
