...
export const CardsData2 = () => {
    ...
    // Filtrar contratos del usuario actual
    const userContratos = contratos.filter(contrato => contrato.usuarioArrendador?.id === Number(userId) || contrato.usuarioArrendatario?.id === Number(userId));
    ...
    return (
        <div className="w-full h-[150px] flex flex-row items-center justify-between bg-[#EFEDDE] border-[1px] border-[#BCBBB0] rounded-[30px]">
            <div className="w-[70%] flex flex-col items-start justify-center p-6 gap-1 max-[1082px]:p-2">
                ...
                <span className="text-[#393939] text-[30px] font-bold max-[1082px]:text-[25px] max-[708px]:text-[20px]">{userContratos.length}</span>
